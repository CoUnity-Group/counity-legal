...
When you use a Discord server, Discord Inc. is the platform operator and processes your data under [its own Privacy Policy](https://discord.com/privacy). We receive a limited subset of that data through Discord's API, and only for the servers our bots have been added to.
...
## 3. Wallet linking, and what it does and does not mean

Where you link a blockchain wallet, we verify that you control it — for example by asking you to place a short code in a public profile, or to send a small self-transfer of a specified amount to your own address. We then record the association between your Discord account and that address so features such as role eligibility can work.

Please understand:

- We take **no custody** of your assets and cannot move, spend or freeze them.
- We only ever read **public** blockchain data.
- A wallet address is public on its chain. What is not public is the **link between that address and your Discord account** — and that link is created by you, held by us, and is one of the records we protect most carefully.
- Linking a wallet is optional. If you do not want that association to exist, do not link a wallet, or ask us to remove it (see Section 8).
...
| Purpose | Legal basis |
|---|---|
| Operating features you actively invoke | Performance of a contract, or your consent |
| Wallet address, email, profile handles you submit | Your consent (Art. 6(1)(a)) |
| Moderation, safety, anti-abuse, audit records | Legitimate interests (Art. 6(1)(f)) |
| Aggregate reporting to administrators | Legitimate interests (Art. 6(1)(f)) |
| Retaining records to meet a legal obligation | Legal obligation (Art. 6(1)(c)) |

Where we rely on consent, you may withdraw it at any time — see Section 8. Withdrawing consent does not affect processing that already took place, and may mean a feature can no longer work for you.
...
### 5.1 With the server you are in
...
### 5.2 With service providers acting on our behalf
...
|---|---|
| Cloud hosting and databases | All stored records, and application logs |
| Workflow and record-keeping platforms | Application and program data, which may include your Discord username, email address, wallet address and submitted profile links |
| Team communication tools | Escalation records, including message excerpts and moderator notes |
| Messaging platform bridges | Message content, author name and attachments from bridged channels |
| Blockchain data providers | Wallet addresses, in order to read public on-chain data during verification and eligibility checks |
| AI and language model providers | Message content and context, for conversational assistant features |

Some of these providers are located in the United States and elsewhere.

### 5.3 With program partners
...
Where we make such transfers we rely on appropriate safeguards, including the European Commission's Standard Contractual Clauses or another lawful transfer mechanism, together with the security measures in Section 7. You may request further information about these safeguards at privacy@counity.xyz.
...
<!-- DO NOT ADD THE FOLLOWING BULLET UNTIL IT IS ACTUALLY TRUE IN PRODUCTION.
     Once the column-level encryption work is complete in every bot that stores
     wallet links — i.e. the plaintext columns have been dropped, not merely
     shadowed by ciphertext — add:

  - Additional application-level encryption of the most sensitive records,
    notably the association between a Discord account and a blockchain wallet
    address, so those values are not readable from a database copy alone

     Likewise, once every personal-data export writes an audit record, extend the
     export bullet with "and recording when such an export is taken".

     Claiming a security control you do not yet have is a misrepresentation and a
     real liability. Ship the control first, then the sentence. -->
...
## 8. Your choices: opting out and deleting your data

### 8.1 The simple options
...
### 8.2 Opting out, and requesting deletion, by email
...
1. The subject line **"Data Deletion Request"** or **"Opt-Out Request"**
2. **Your Discord user ID** (the numeric ID — in Discord, enable Developer Mode under Settings → Advanced, then right-click your name and choose "Copy User ID")
3. The Discord server(s) you used our Services in, if you know them
4. Whether you want **all** your data deleted, or only specific data such as a linked wallet or an email address
...
- We cannot retract information already shared with a program partner under Section 5.3; we will tell you who received it so you can contact them, and we will pass your request on.
...
### 8.3 Your rights under the GDPR / UK GDPR
...
### 8.4 Your rights under the CCPA / CPRA
...
To exercise any of these rights, use the email process in Section 8.2.
...
When our Services are removed from a server, or when you make a verified deletion request, we delete or anonymise the associated records as described in Section 8.2.
